...
CLASS ltcl_fake_response IMPLEMENTATION.
  ...
  METHOD set_header_field.
    ...
    DATA temp3 TYPE zabaputil_cl_util_context=>ty_s_name_value.
    CLEAR temp3.
    temp3-n = name.
    temp3-v = value.
...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD test_factory_cloud.
    ...
    DATA lo_cut TYPE REF TO zabaputil_cl_util_http.
    ...
    lo_cut = zabaputil_cl_util_http=>factory_cloud( req = temp1
                                                      res = temp2 ).
  ...
  METHOD test_get_header_field.
    ...
    DATA temp6 TYPE zabaputil_cl_util_context=>ty_s_name_value.
    CLEAR temp6.
    temp6-n = `~path`.
  ...
  METHOD test_set_header_crlf_val.
    ...
    DATA lv_nl LIKE zabaputil_cl_util_context=>cv_char_util_cr_lf.
    DATA temp9 LIKE LINE OF mo_server->response->mt_header.
    DATA temp10 LIKE sy-tabix.
    lv_nl = zabaputil_cl_util_context=>cv_char_util_cr_lf.
    ...
    temp10 = sy-tabix.
    READ TABLE mo_server->response->mt_header WITH KEY n = `x-app` INTO temp9.
    sy-tabix = temp10.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_set_header_crlf_name.
    ...
    DATA lv_nl LIKE zabaputil_cl_util_context=>cv_char_util_cr_lf.
    DATA temp11 LIKE LINE OF mo_server->response->mt_header.
    ...
    temp12 = sy-tabix.
    READ TABLE mo_server->response->mt_header WITH KEY n = `x-appx-second: 1` INTO temp11.
    sy-tabix = temp12.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_set_header_lf_only.
    ...
    DATA temp13 LIKE LINE OF mo_server->response->mt_header.
    DATA temp14 LIKE sy-tabix.
    lv_lf = zabaputil_cl_util_context=>cv_char_util_cr_lf+1(1).
    ...
    temp14 = sy-tabix.
    READ TABLE mo_server->response->mt_header WITH KEY n = `x-app` INTO temp13.
    sy-tabix = temp14.
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
  ...
  METHOD test_get_response_cookie.
    ...
    DATA temp15 TYPE zabaputil_cl_util_context=>ty_s_name_value.
    CLEAR temp15.
    temp15-n = `sap-sessionid`.
    temp15-v = `ABC123`.
    INSERT temp15 INTO TABLE mo_server->response->mt_cookie.
  ...
  METHOD test_onprem_getter_cache.
    DATA lo_response_old LIKE mo_server->response.
    ...
    lo_response_old = mo_server->response.
    CREATE OBJECT mo_server->request.
    CREATE OBJECT mo_server->response.
