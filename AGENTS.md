...
4. **Periodic AI sync-back:** every few weeks an AI compares abap-util with all consumers and merges what was added or fixed downstream into this repository — so abap-util always converges back to the superset of all methods, unit-tested and linted for all targets, and every other consumer can pick them up from here.
   ...
   **A method the master has to keep wider than the consumer's is not drift.** A consumer trims its copy, so a symbol its trim dropped will show up as a body difference forever. Known and settled: `class_constructor` keeps `cv_char_util_charsize`, which abap2UI5 dropped because its trimmed copy has no caller for it and the catalogue has two (the XLSX column-width calculation). Record such a case here when it is decided, or the next run re-opens it.
